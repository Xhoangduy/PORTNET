                  ...
                  <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 md:p-8">
                      ...
                      <div className="mb-8">
                         <h3 className="text-blue-700 font-bold text-sm uppercase mb-4 pb-2 border-b border-gray-100 flex items-center">
                             <Wrench className="w-4 h-4 mr-2" /> 1. Thông tin lệnh dịch vụ
                         </h3>
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-xs font-bold text-gray-900 uppercase mb-1.5">Loại chứng từ <span className="text-red-500">*</span></label>
                                    <div className="flex gap-4">
                                        <label className={`flex-1 flex items-center cursor-pointer p-3 border rounded-lg transition-colors ${docType === 'bl' ? 'border-blue-500 bg-white' : 'border-gray-300 bg-white hover:bg-gray-50'}`}>
                                            <input type="radio" checked={docType === 'bl'} onChange={() => setDocType('bl')} className="text-blue-600 focus:ring-blue-500" />
                                            <span className={`ml-2 text-sm font-medium ${docType === 'bl' ? 'text-blue-700' : 'text-gray-900'}`}>Số Vận Đơn (B/L)</span>
                                        </label>
                                        <label className={`flex-1 flex items-center cursor-pointer p-3 border rounded-lg transition-colors ${docType === 'booking' ? 'border-blue-500 bg-white' : 'border-gray-300 bg-white hover:bg-gray-50'}`}>
                                            <input type="radio" checked={docType === 'booking'} onChange={() => setDocType('booking')} className="text-blue-600 focus:ring-blue-500" />
                                            <span className={`ml-2 text-sm font-medium ${docType === 'booking' ? 'text-blue-700' : 'text-gray-900'}`}>Số Booking</span>
                                        </label>
                                    </div>
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-gray-900 uppercase mb-1.5">Số {docType === 'bl' ? 'Vận đơn' : 'Booking'} <span className="text-red-500">*</span></label>
                                    <input 
                                        type="text" 
                                        value={docNumber}
                            ...
                            <div className="space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="col-span-2">
                                        <label className="block text-xs font-bold text-gray-900 uppercase mb-1.5">Loại dịch vụ <span className="text-red-500">*</span></label>
                                        <select 
                                            value={serviceType}
                                        ...
                                        >
                                            <option value="vesinh">Vệ sinh Container (Cleaning)</option>
                                            <option value="suachua">Sửa chữa Container (Repair)</option>
                                            <option value="camdien">Cắm điện (Power Supply)</option>
                  ...
                  <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden min-h-[500px] flex flex-col">
                    <div className="p-6 border-b border-gray-200 flex justify-between items-center bg-gray-50/50">
                        <div>
                             ...
                             <p className="text-xs text-gray-500 mt-1">Chọn container để thực hiện dịch vụ: {getServiceLabel(serviceType)}</p>
                        </div>
                            ...
                            <table className="w-full text-sm text-left bg-white">
                                <thead className="text-xs text-gray-500 uppercase bg-gray-100 border-b border-gray-200 sticky top-0 z-10">
                                    <tr>
                                        ...
                                        <th className="px-6 py-4 text-right">Phí ước tính (VND)</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-100">
                                    {containers.map((cont) => (
                                        <tr key={cont.id} className={`hover:bg-blue-50/50 transition-colors ${selectedContainerIds.includes(cont.id) ? 'bg-blue-50/30' : ''}`}>
                                            ...
                                            <td className="px-6 py-4 text-gray-900">{cont.isoSize}</td>
                                            <td className="px-6 py-4 font-mono text-gray-500">{cont.location}</td>
                                            <td className="px-6 py-4 text-blue-600 font-medium">{cont.serviceName}</td>
                                 ...
                                 <tbody className="divide-y divide-gray-100 bg-white">
                                     {selectedContainers.map((cont) => (
                                        <tr key={cont.id} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-4 py-3 text-gray-900">{cont.serviceName}</td>
                                            <td className="px-4 py-3 text-center text-gray-900">{cont.containerNo}</td>
              ...
              {currentStep === 4 && (
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
                    ...
                    <p className="text-gray-500 max-w-md mx-auto mb-8 leading-relaxed">
                        Lệnh dịch vụ đã được khởi tạo thành công. Mã giao dịch của bạn là <span className="font-mono font-bold text-gray-800">SVC-2023-9988</span>. 
